    ...
    <>
      <Row className='align-items-center'>
        <Col sm={4} className='flex-[1]'>
          <img className="imagee object-fit-fill" src={article.pic} alt="" style={{ height: '280px' }} />
        </Col>
        <Col sm={8} className="flex-[2.5] d-flex flex-column justify-content-center h-100">
          ...
          <p className=" ms-2 pt-4 mb-1 " style={{ textAlign: 'justify' }}>{article.content} <Link className="text-reset link-underline-dark link-danger" to={`/article/${article.id}`}>+ more</Link></p>
  ...
  const renderRightLayout = () => (
    <>
      <Col sm={8} className="flex-[2.5] d-flex flex-column justify-content-center h-100">
        ...
        <Row className='d-flex flex-row align-items-center'>
          <div className="col-1">
            <img className="object-fit-fill rounded-full author-img" src={article.pic} alt="" />
          </div>
          <div className="col-6">
            <h4 className='author'>Published by Author <span className='name'>{article.author}</span></h4>
          </div>
